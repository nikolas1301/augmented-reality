...
[verts edges faces] = createSoccerBall;
verts = verts.*30;
...
while(true)
    ...
    for i=1:size(faces,1)
        if size(faces{i},2) == 6
            patch('Faces',faces{i},'Vertices',vertsUV,'FaceColor','w','LineStyle', '-')
        else
            patch('Faces',faces{i},'Vertices',vertsUV,'FaceColor','k','LineStyle', '-')
        end
    end
